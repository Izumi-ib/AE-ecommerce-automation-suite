...
Feature: User registration and account deletion
  ...
  Scenario Outline: User registers without email and
    When user enters name '<name>' and email '<email>' and clicks Signup button
    Then user should see browser alert and validate the message '<expectedMessage>'

    Examples:
      | name          | email      | expectedMessage             |
      | username      | emptyEmail | Please fill out this field. |
      | emptyUsername | password   | Please fill out this field. |
